...
$(document).ready(function () {
    ...
    function choose_message(rhyme_type: string) {
        if (rhyme_type === "consonante") {
            var word =
                conson_words[Math.floor(Math.random() * conson_words.length)];
        } else {
            var word =
                asson_words[Math.floor(Math.random() * asson_words.length)];
        }
    ...
    function createInput(rhymeKey: string, rhymeType: string) {
        ...
        input.change(function () {
            ...
            if (/^-?[a-zA-ZñÑÓóÁáÉéíÍúÚü]+$/.test(inputVal)) {
                $.ajax({
                    ...
                    success: function (resp_data) {
                        if (resp_data.not_valid) {
                            ...
                            validInvalid(
                                $input,
                                "La rima es válida y existe en la base de datos.",
                                true
                            );
                        }
                    },
                ...
                let err_msg =
                    "Solo se permiten caracteres alfabéticos y el guión.";
    ...
    function resetToNeutral(inputElem: JQuery<HTMLElement>, msg: string) {
    ...
    function rhySeqIsValid(): boolean {
        ...
        if (
            ...
            let err_msg =
                "Solo valen caracteres del abecedario y espacios en blanco";
        ...
        let err_msg =
            "El número de caracteres ha de coincidir con el número de versos (sin contar espacios)";
        validInvalid($rhySeq, err_msg, false);
        selVerIsValid();
        return false;
    }
    ...
    function verNumIsValid(): boolean {
    ...
    function verLenIsValid(): boolean {
    ...
    $(".popover").each(function () {
        ...
        if (text === "Número de versos:") {
            var message =
                "Un número entre 1 y 20 para que germine un poema. Si se deja en blanco será un número aleatorio de versos.";
            var where = "top";
        } else if (text === "Longitud de los versos:") {
            var message =
                "Longitud de los versos, medidos en número de sílabas. Si el campo queda vacío serán versos de tamaño variable.";
            var where = "top";
        } else {
            var message =
                "Mayúsculas: rima consonante. Minúsculas: asonante. Espacio en blanco: verso en blanco. El campo vacío producirá poemas sin rima.";
            var where = "bottom";
        }
    ...
    $verNum.on("change, blur", function () {
        ...
        if (typeof verNumVal === "string") {
            if (verNumVal === "") resetToNeutral($(this), "Min: 1, Max: 20");
            else if (0 < +verNumVal && +verNumVal < 21) {
                validInvalid(
                    $(this),
                    "Esto número tiene buena pinta.",
                    true
                ); /* Tiene buena pinta */
            } else
                validInvalid($(this), "Sólo números. Min: 1, Max: 20", false);

            if ($rhySeq.val() !== "") rhySeqIsValid();
        }
    });
